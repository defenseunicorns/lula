...
{#if isOpen}
	...
	>
		<div class="absolute inset-0 overflow-hidden">
			<!-- Backdrop -->
			<div
				class="absolute inset-0 bg-gray-600 dark:bg-gray-900 bg-opacity-75 dark:bg-opacity-75 transition-opacity"
				onclick={onClose}
								...
								{#if currentAdapter}
									...
									>
										...
										<div class="text-xs text-blue-700 dark:text-blue-300">
											{currentAdapter.schema.fields.length} fields across {currentAdapter.schema
												.groups?.length || 1} sections
										</div>
											...
											<div class="mt-2 space-y-1">
												{#each currentAdapter.schema.groups as group}
													<div class="text-xs text-blue-600 dark:text-blue-300">
														• {group.label}: {currentAdapter.schema.fields.filter(
															(f) => f.group === group.id
														).length} fields
													</div>
												{/each}
