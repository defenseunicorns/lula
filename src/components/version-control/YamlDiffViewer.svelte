<script lang="ts">
  ...
  function getTextDiff(oldText: string, newText: string) {
    ...
    oldHighlighted = oldChanges.map(({ token, changed }) => 
      changed ? `<mark class="bg-red-200 dark:bg-red-800 text-red-900 dark:text-red-100 px-0.5 rounded">${escapeHtml(token)}</mark>` : escapeHtml(token)
    ).join('');
    
    newHighlighted = newChanges.map(({ token, changed }) => 
      changed ? `<mark class="bg-green-200 dark:bg-green-800 text-green-900 dark:text-green-100 px-0.5 rounded">${escapeHtml(token)}</mark>` : escapeHtml(token)
    ).join('');
...
<div class="yaml-diff-viewer bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm overflow-hidden">
              ...
              <div class="flex items-start space-x-3">
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-center space-x-2 mb-1">
                    ...
                    <span class="text-xs text-gray-500 dark:text-gray-400 font-mono bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded">
                      {change.path}
                    </span>
                  </div>
          ...
          {#each yamlDiff.changes as change}
            <div class="flex items-center space-x-2 text-sm">
              ...
              {#if change.type === 'modified' && change.oldValue !== change.newValue}
                {@const diff = getTextDiff(formatValue(change.oldValue), formatValue(change.newValue))}
                {@const oldTruncated = truncateWithHighlight(diff.oldHighlighted || escapeHtml(formatValue(change.oldValue)), 80)}
                {@const newTruncated = truncateWithHighlight(diff.newHighlighted || escapeHtml(formatValue(change.newValue)), 80)}
                <div class="flex items-center space-x-2 text-xs ml-2 mt-1">
                  <span class="text-red-600 dark:text-red-400 font-mono">−</span>
                  <code class="text-red-600 dark:text-red-400 bg-red-100 dark:bg-red-900/30 px-2 py-1 rounded">
                    {@html oldTruncated}
                  </code>
                </div>
                <div class="flex items-center space-x-2 text-xs ml-2 mt-1">
                  <span class="text-green-600 dark:text-green-400 font-mono">+</span>
                  <code class="text-green-600 dark:text-green-400 bg-green-100 dark:bg-green-900/30 px-2 py-1 rounded">
                    {@html newTruncated}
                  </code>
                </div>
