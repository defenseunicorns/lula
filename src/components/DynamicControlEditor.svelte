<script lang="ts">
  ...
  interface Props {
    ...
    useDynamicForms?: boolean; // Feature flag for dynamic forms
  }
  ...
  const detailsFields = $derived(schema.fields.filter(f => 
    f.group === 'identification' || f.group === 'description'
  ));
  
  const implementationFields = $derived(schema.fields.filter(f => 
    f.group === 'implementation'
  ));
  
  const complianceFields = $derived(schema.fields.filter(f => 
    f.group === 'compliance'
  ));
  ...
  const detailsErrors = $derived(
    validationResult?.errors.filter(e => 
      detailsFields.some(f => f.id === e.field)
    ).length || 0
  );
  
  const implementationErrors = $derived(
    validationResult?.errors.filter(e => 
      implementationFields.some(f => f.id === e.field)
    ).length || 0
  );
  
  const complianceErrors = $derived(
    validationResult?.errors.filter(e => 
      complianceFields.some(f => f.id === e.field)
    ).length || 0
  );
</script>
...
<div class="fixed inset-0 bg-gray-600 dark:bg-gray-900 bg-opacity-50 dark:bg-opacity-75 overflow-y-auto h-full w-full z-50" role="dialog" aria-modal="true" tabindex="-1" onclick={onClose} onkeydown={(e) => e.key === 'Escape' && onClose()}>
  ...
  <div class="relative min-h-screen w-full p-6 bg-white dark:bg-gray-800" role="document" onclick={(e) => e.stopPropagation()} onkeydown={(e) => e.stopPropagation()}>
    ...
    <div class="mt-4">
      <nav class="flex space-x-8">
        ...
        <button
          ...
          class="py-2 px-1 border-b-2 font-medium text-sm {activeTab === 'implementation' ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'}"
        >
          Implementation
        ...
        <button
          ...
          class="py-2 px-1 border-b-2 font-medium text-sm {activeTab === 'compliance' ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'}"
        >
          Compliance
    ...
    <div class="mt-6 max-h-screen-3/4 overflow-y-auto">
      {#if activeTab === 'details' && useDynamicForms}
        <DynamicControlForm
        ...
        />
      {:else if activeTab === 'implementation' && useDynamicForms}
        <DynamicControlForm
          bind:control={editedControl}
          schema={{
            ...schema,
        ...
        {#if ccisInNarrative.length > 0}
          <div class="mt-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
            <div class="block text-sm font-medium text-blue-700 dark:text-blue-300 mb-2">CCIs Found in Narrative</div>
        ...
        {/if}
      {:else if activeTab === 'compliance' && useDynamicForms}
        <DynamicControlForm
          bind:control={editedControl}
          schema={{
            ...schema,
