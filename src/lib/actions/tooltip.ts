...
export function tooltip(node: HTMLElement) {
  ...
  function updatePosition() {
    ...
    if (!hasPositionClass) {
      ...
      const position = useVerticalPosition ? `tooltip-${verticalPosition}` : `tooltip-${horizontalPosition}`

      tooltipEl.className = tooltipEl.className.replace(/tooltip-(left|right|top|bottom)/g, '') + ' ' + position
    }
  }
