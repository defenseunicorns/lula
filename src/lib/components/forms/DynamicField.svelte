	...
	{:else if field.type === 'string-array'}
		<div class="space-y-3">
			{#if ensureArray().length === 0 && !readonly}
				...
				<div class="flex flex-col items-center justify-center py-8 px-4 bg-gray-50 dark:bg-gray-800 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg">
					<svg class="w-8 h-8 text-gray-400 dark:text-gray-500 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
						<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"></path>
					</svg>
					<p class="text-sm text-gray-500 dark:text-gray-400 mb-3">No {field.label.toLowerCase()} added yet</p>
					<button
						type="button"
			...
			{:else}
				{#each ensureArray() as item, index}
					<div class="group relative p-4 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm hover:shadow-md transition-shadow">
						...
						{#if field.id === 'objectives' || field.id === 'guidance' || field.id === 'references' || (field.placeholder && field.placeholder.length > 50)}
							<textarea
								value={item}
								placeholder={field.placeholder || `Enter ${field.label.replace(/s$/, '').toLowerCase()}`}
								rows="3"
	...
	{:else if field.type === 'object-array'}
		<div class="space-y-4">
			{#if ensureArray().length === 0 && !readonly}
				...
				<div class="flex flex-col items-center justify-center py-10 px-4 bg-gray-50 dark:bg-gray-800 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg">
					<svg class="w-10 h-10 text-gray-400 dark:text-gray-500 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
						<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"></path>
					</svg>
					<p class="text-sm text-gray-500 dark:text-gray-400 mb-4 text-center">No {field.label.toLowerCase()} created yet</p>
					<button
						type="button"
			...
			{:else}
				{#each ensureArray() as item, index}
					<div class="group relative bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm hover:shadow-md transition-all duration-200">
						<!-- Card header -->
						<div class="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 rounded-t-xl">
							<div class="flex items-center space-x-3">
								<!-- Drag handle -->
								<div class="cursor-move text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
							...
							{#if field.arraySchema}
								<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
									{#each Object.entries(field.arraySchema) as [key, schemaObj]}
										...
										<div class="space-y-2">
											<label for="{field.id}-{index}-{key}" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
				...
				{#if !readonly}
					...
					>
						<div class="flex items-center space-x-3">
							<div class="p-2 bg-blue-100 dark:bg-blue-800 rounded-full group-hover:bg-blue-200 dark:group-hover:bg-blue-700 transition-colors">
								<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
									<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
								</svg>
							</div>
