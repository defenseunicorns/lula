<script lang="ts">
  ...
  let fieldConfigs = new Map<string, {
    originalName: string;
    tab: TabAssignment;
    displayOrder: number;
    ...
    if (rawData.length > headerRowIndex) {
      ...
      if (headers && headers.length > 0) {
        ...
        fields.forEach((field, index) => {
          ...
          if (lowerField.includes('implementation') || lowerField.includes('status') || 
              lowerField.includes('narrative') || lowerField.includes('guidance')) {
            tab = 'implementation';
          } else if (lowerField.includes('id') || lowerField.includes('title') || 
                     lowerField.includes('family') || lowerField.includes('cci') ||
                     lowerField.includes('control') || lowerField.includes('acronym')) {
            tab = 'overview';
          }
          
          // Smart field type detection
          if (lowerField.includes('description') || lowerField.includes('narrative') || 
              lowerField.includes('guidance') || lowerField.includes('statement')) {
            fieldType = 'textarea';
          } else if (lowerField.includes('status') || lowerField.includes('type') || 
        ...
        if (!controlIdField && fields.includes('AP Acronym')) {
          const hasShortValues = !sampleData.length || 
          ...
          const hasUniqueValues = !nonEmptyValues.length || uniqueValues.size === nonEmptyValues.length;
  ...
  function handleTabDrop(e: DragEvent, tab: TabAssignment, targetIndex?: number) {
    ...
    if (draggedField && fieldConfigs.has(draggedField)) {
      ...
      if (targetIndex !== undefined && tab !== null) {
        ...
        const maxOrder = Math.max(0, ...Array.from(fieldConfigs.values())
...
<div class="space-y-6">
  ...
  {#if fileName}
    <div class="p-4 text-sm text-blue-800 rounded-lg bg-blue-50 dark:bg-gray-800 dark:text-blue-400">
      <div class="flex items-center">
        ...
        <div>
          <span class="font-medium">File loaded:</span> {fileName}
          <div class="mt-1">
            <span class="font-medium">Sheets:</span> {sheets.length} | 
            <span class="font-medium">Fields:</span> {fields.length} | 
            <span class="font-medium">Controls found:</span> {controlCount}
          </div>
        </div>
      </div>
    </div>
  ...
  {#if showFieldMapping && fields.length > 0}
    <!-- Import Options -->
    <div class="bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <label for="headerRow" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
            Select Header Row
          </label>
          <select 
            id="headerRow"
        ...
        <div>
          <label for="controlIdField" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
            Control ID Field <span class="text-red-500">*</span>
          </label>
          <select 
            id="controlIdField"
            bind:value={controlIdField}
            class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:text-white {!controlIdField ? 'border-red-500' : ''}"
            required
          >
            <option value="" disabled>Select Control ID field</option>
            {#each fields as field}
              {@const exampleValue = sampleData.length > 0 && sampleData[0][field] ? 
    ...
    <div class="bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
      ...
      <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">
        Drag fields from Available Fields into tabs to organize how they'll appear in the control details panel
      </p>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-4 gap-4">
        ...
        <div class="border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800">
          <div class="p-3 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700 rounded-t-lg">
          ...
          >
            ...
            {#if availableFields.filter(f => !fieldConfigs.get(f) || fieldConfigs.get(f)?.tab === null).length === 0}
              <p class="text-xs text-gray-400 dark:text-gray-500 text-center py-4">No excluded fields</p>
            {/if}
          </div>
        </div>

        <!-- Overview Tab Column -->
        <div class="border border-blue-300 dark:border-blue-700 rounded-lg bg-white dark:bg-gray-800">
          <div class="p-3 border-b border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-900/20 rounded-t-lg">
            <h4 class="text-sm font-semibold text-blue-700 dark:text-blue-300">Overview Tab</h4>
            <p class="text-xs text-blue-600 dark:text-blue-400 mt-1">Core control information</p>
          </div>
          <div 
            class="p-3 min-h-[400px] max-h-[600px] overflow-y-auto space-y-2 transition-colors
          ...
          >
            {#each Array.from(fieldConfigs.entries())
              ...
              <div 
                ...
                class="flex items-center px-3 py-2 bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300 rounded text-sm cursor-move hover:bg-blue-200 dark:hover:bg-blue-800/30 transition-colors
                  {dragOverField === field && draggedField !== field ? 'border-t-2 border-blue-500' : ''}"
              >
                <svg class="w-3 h-3 mr-2 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                  <path d="M10 6a2 2 0 110-4 2 2 0 010 4zM10 12a2 2 0 110-4 2 2 0 010 4zM10 18a2 2 0 110-4 2 2 0 010 4z"/>
                </svg>
            ...
            {#if Array.from(fieldConfigs.entries()).filter(([field, config]) => config.tab === 'overview').length === 0}
              <p class="text-xs text-gray-400 dark:text-gray-500 text-center py-4">Drop fields here</p>
            {/if}
          </div>
        </div>

        <!-- Implementation Tab Column -->
        <div class="border border-green-300 dark:border-green-700 rounded-lg bg-white dark:bg-gray-800">
          <div class="p-3 border-b border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-900/20 rounded-t-lg">
            <h4 class="text-sm font-semibold text-green-700 dark:text-green-300">Implementation Tab</h4>
            ...
            {#each Array.from(fieldConfigs.entries())
              ...
              <div 
                ...
                class="flex items-center px-3 py-2 bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300 rounded text-sm cursor-move hover:bg-green-200 dark:hover:bg-green-800/30 transition-colors
                  {dragOverField === field && draggedField !== field ? 'border-t-2 border-green-500' : ''}"
              >
                <svg class="w-3 h-3 mr-2 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                  <path d="M10 6a2 2 0 110-4 2 2 0 010 4zM10 12a2 2 0 110-4 2 2 0 010 4zM10 18a2 2 0 110-4 2 2 0 010 4z"/>
                </svg>
        ...
        <!-- Custom Tab Column -->
        <div class="border border-purple-300 dark:border-purple-700 rounded-lg bg-white dark:bg-gray-800">
          <div class="p-3 border-b border-purple-200 dark:border-purple-800 bg-purple-50 dark:bg-purple-900/20 rounded-t-lg">
            ...
            {#each Array.from(fieldConfigs.entries())
              ...
              <div 
                ...
                class="flex items-center px-3 py-2 bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-300 rounded text-sm cursor-move hover:bg-purple-200 dark:hover:bg-purple-800/30 transition-colors
                  {dragOverField === field && draggedField !== field ? 'border-t-2 border-purple-500' : ''}"
              >
                <svg class="w-3 h-3 mr-2 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                  <path d="M10 6a2 2 0 110-4 2 2 0 010 4zM10 12a2 2 0 110-4 2 2 0 010 4zM10 18a2 2 0 110-4 2 2 0 010 4z"/>
                </svg>
    ...
    {#if sampleData.length > 0}
      <div class="bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Sample Data Preview</h3>
