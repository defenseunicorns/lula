...
router.get('/mappings/:controlId/history', async (req: Request, res: Response) => {
  try {
    ...
    const mappingFilePath = join(state.CONTROL_SET_DIR, 'mappings', family, `${controlId}-mappings.yaml`);
...
router.get('/controls/:id/complete', async (req: Request, res: Response) => {
  try {
    ...
    // Get file paths
    const metadata = state.fileStore.getControlMetadata(controlId);
    const controlFilePath = metadata ? join(state.CONTROL_SET_DIR, 'controls', family, metadata.filename) : undefined;
    const mappingFilePath = join(state.CONTROL_SET_DIR, 'mappings', family, `${controlId}-mappings.yaml`);
    ...
    const unifiedHistory: UnifiedHistory = {
      ...
      controlCommits,
      mappingCommits,
      controlFilePath: controlFilePath ? controlFilePath.replace(state.CONTROL_SET_DIR + '/', '') : undefined,
      mappingFilePath: mappingFilePath ? mappingFilePath.replace(state.CONTROL_SET_DIR + '/', '') : undefined
    };
