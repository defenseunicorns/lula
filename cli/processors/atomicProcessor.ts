...
export class AtomicProcessor {
	...
	async processOSCAL(
		...
		await this.saveFrameworkMetadata(
			framework,
			uniqueControls,
			options.output_dir,
			options.dry_run || false
	 ...
	 */
	private async processCatalogWithCCI(
		catalog: any,
		options: AtomicImportOptions
	): Promise<AtomicControl[]> {
	 ...
	 */
	private async processCatalogAsControls(
		catalog: any,
		options: AtomicImportOptions
	): Promise<AtomicControl[]> {
		const controls: AtomicControl[] = [];
	...
	private convertOSCALToAtomic(
		...
		const atomicControl: AtomicControl = {
			...
			properties: simpleMode
				? this.extractSimpleProperties(oscalControl)
				: oscalControl.props
					? this.convertProps(oscalControl.props)
					: undefined
		};
		...
		if (oscalControl.links && oscalControl.links.length > 0 && !simpleMode) {
			...
			const externalLinks = oscalControl.links.filter((link: any) => !link.href?.startsWith('#'));
	 ...
	 */
	private enrichEnhancedControlWithOSCAL(
		enhancedControl: any,
		oscalControl: any,
		options?: { resolveParameters?: boolean; simpleMode?: boolean }
	): void {
		...
		const objectives = this.extractObjectivesAsStrings(
			oscalControl,
			resolveParams,
			enhancedControl
		);
	...
	private async saveControls(
		...
		for (const [nistControlId, groupedControls] of controlGroups.entries()) {
			...
			const family =
				(groupedControls[0] as any)?._metadata?.family?.toLowerCase() ||
				groupedControls[0].family ||
				'unknown';
	...
	private groupControlsByNISTControl(controls: AtomicControl[]): Map<string, AtomicControl[]> {
		...
		for (const control of controls) {
			...
			const nistControlId =
				(control as any)['control-acronym'] || (control as any).parent_control || control.id;
	 ...
	 */
	private extractStatementParts(
		oscalControl: any,
		resolveParams: boolean = false
	): any[] | undefined {
		...
		return statementPart.parts.map((part: any) =>
			this.convertPartToSimpleStructure(part, oscalControl.params, resolveParams)
		);
	}
	 ...
	 */
	private extractAssessmentMethods(
		oscalControl: any,
		resolveParams: boolean = false
	): any[] | undefined {
		if (!oscalControl.parts) return undefined;
		...
		return assessmentParts.map((part: any) =>
			this.convertPartToSimpleStructure(part, oscalControl.params, resolveParams)
		);
	}
	 ...
	 */
	private extractObjectivesAsStrings(
		oscalControl: any,
		resolveParams: boolean = false,
		cciControl?: any
	): any[] | undefined {
		...
		if (cciControl && cciControl.cci) {
			const specificObjectives = this.extractCCISpecificObjectives(
				objectivePart,
				cciControl,
				resolveParams,
				oscalControl.params
		...
		if (objectivePart.prose) {
			objectives.push(
				this.cleanTextForBackupFormat(objectivePart.prose, resolveParams, oscalControl.params)
			);
		}
		...
		if (objectivePart.parts && objectivePart.parts.length > 0) {
			for (const subPart of objectivePart.parts) {
				if (subPart.prose) {
					objectives.push(
						this.cleanTextForBackupFormat(subPart.prose, resolveParams, oscalControl.params)
					);
				}
				...
				if (subPart.parts && subPart.parts.length > 0) {
					for (const nestedPart of subPart.parts) {
						if (nestedPart.prose) {
							objectives.push(
								this.cleanTextForBackupFormat(nestedPart.prose, resolveParams, oscalControl.params)
							);
						}
					}
				}
	 ...
	 */
	private extractCCISpecificObjectives(
		objectivePart: any,
		cciControl: any,
		...
		const nistRef = cciItem.references.find(
			(ref) => ref.creator === 'NIST' && ref.title.includes('SP 800-53') && ref.version === '4'
		);
	 ...
	 */
	private extractStatementAsStrings(
		oscalControl: any,
		resolveParams: boolean = false
	): any[] | undefined {
		if (!oscalControl.parts) return undefined;
		...
		const structuredParts = this.extractStructuredPartsFromOSCAL(
			statementPart,
			oscalControl.params,
			resolveParams
		);
	 ...
	 */
	private extractStructuredPartsFromOSCAL(
		part: any,
		parameters?: any[],
		resolveParams: boolean = false
	): any[] {
		...
		if (part.prose) {
			...
			if (proseText) {
				...
				if (part.parts && part.parts.length > 0) {
					...
					const subParts = this.extractStructuredPartsFromOSCAL(
						{ parts: part.parts },
						parameters,
						resolveParams
					);
	 ...
	 */
	private traverseObjectiveForCCI(
		part: any,
		cciIndex: string,
		objectives: any[],
		resolveParams: boolean,
	 ...
	 */
	private buildStructuredObjectiveText(
		part: any,
		objectives: any[],
		resolveParams: boolean,
		parameters?: any[],
	 ...
	 */
	private extractAssessmentMethodsAsStrings(
		oscalControl: any,
		resolveParams: boolean = false
	): string[] | undefined {
		if (!oscalControl.parts) return undefined;
		...
		for (const assessmentPart of assessmentParts) {
			if (assessmentPart.prose) {
				methods.push(
					this.cleanTextForBackupFormat(assessmentPart.prose, resolveParams, oscalControl.params)
				);
			}
	 ...
	 */
	private cleanTextForBackupFormat(
		text: string,
		resolveParams: boolean,
		parameters?: any[]
	): string {
	 ...
	 */
	private convertPartToSimpleStructure(
		part: any,
		parameters?: any[],
		resolveParams: boolean = false
	): any {
		...
		if (part.parts && part.parts.length > 0) {
			result.parts = part.parts.map((p: any) =>
				this.convertPartToSimpleStructure(p, parameters, resolveParams)
			);
		}
	...
	private extractTextFromPart(
		...
		if (Array.isArray(part.parts)) {
			const subTexts = part.parts.map((p: any) =>
				this.extractTextFromPart(p, parameters, resolveParams)
			);
	 ...
	 */
	private extractSimpleTextFromPart(
		part: any,
		parameters?: any[],
		resolveParams: boolean = false
	): string {
	...
	private deduplicateControls(controls: AtomicControl[]): AtomicControl[] {
		...
		for (const control of controls) {
			...
			if (!existing) {
				...
				const existingFields = Object.keys(existing).filter(
					(k) => existing[k as keyof AtomicControl] != null
				).length;
				const newFields = Object.keys(control).filter(
					(k) => control[k as keyof AtomicControl] != null
				).length;
	...
	private generateFieldSchema(controls: AtomicControl[], cciEnabled: boolean): any {
		...
		const controlsWithAssessmentProcs = controls.filter(
			(c) =>
				(c as any)['assessment-procedures'] && Array.isArray((c as any)['assessment-procedures'])
		);
		...
		const controlsWithAssessmentObjs = controls.filter(
			(c) =>
				(c as any)['assessment-objectives'] && Array.isArray((c as any)['assessment-objectives'])
		);
		...
		fields['compliance-status'] = {
			type: 'string',
			options: ['Not Assessed', 'Satisfied', 'Other Than Satisfied', 'Not Applicable']
		};
		...
		fields['security-control-designation'] = {
			type: 'string',
			options: ['System', 'Hybrid', 'Common', 'Inherited']
		};
