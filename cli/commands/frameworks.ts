...
export class FrameworksCommand {
	...
	private showUsageExamples(): void {
		...
		console.log(
			'  npx tsx cli.ts init --framework=nist-800-53-rev4-moderate --project-name=my-project\n'
		);

		console.log('Import controls directly:');
		console.log(
			'  npx tsx cli.ts import ./my-controls --framework=nist-800-53-rev4-high --with-cci'
		);
		console.log(
			'  npx tsx cli.ts import ./my-controls --framework=nist-800-53-rev4-high --preserve-oscal  # Keep OSCAL structure\n'
		);
