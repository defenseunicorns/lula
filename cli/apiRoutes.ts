...
router.get('/search', (req: Request, res: Response) => {
	try {
		...
		const matchingControls = Array.from(state.controlsCache.values())
			.filter((control) => JSON.stringify(control).toLowerCase().includes(searchTerm))
			.sort((a, b) => a.id.localeCompare(b.id));

		const matchingMappings = Array.from(state.mappingsCache.values())
			.filter((mapping) => JSON.stringify(mapping).toLowerCase().includes(searchTerm))
