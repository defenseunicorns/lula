...
test.describe('Setup Wizard', () => {
  test('default landing (no control sets)', async ({ page }) => {
    ...
    await expect(page.locator('#description'))
      .toContainText("Let's get started by importing a control set from a spreadsheet.", { timeout: 15000 });
  ...
  test('shows tab UI when control sets exist (via CustomEvent)', async ({ page }) => {
    ...
    await page.evaluate(() => {
      window.dispatchEvent(new CustomEvent('control-sets-list', {
        detail: {
          controlSets: [
    ...
    await expect(page.locator('#description'))
      .toContainText('Select an existing control set or import a new one from a spreadsheet.', { timeout: 15000 });
